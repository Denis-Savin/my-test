...
console.log("Hi");
	...
	init = function( event ) {
		...
		canvas.width = container.parentElement.clientWidth + 10;
    ...
    context.fillStyle = "rgba(0, 0, 0,1)";
        ...
        points[i] = { x: (i/pointCount) * (canvas.width+200), y: 100 };
        points[i].o = {x: points[i].x, y: points[i].y };
			} 
    console.log(points); 
...
//        var noise = CanvasPS3.noise(p.o.x+tn, p.o.y+tn, tn);
        var noise = CanvasPS3.noise(p.o.x*0.003+tn, p.o.y*2, tn);
     		p.x = p.x;
        p.y = (noise * 200);
      }
      ...
      context.lineWidth = 0.5;
			...
			for(i = 0; i < points.length; ++i) {
				var hsv = CanvasPS3.HSVRGB((t*360*10) % 360, 60, 100);
